...
### Проблема с отправкой email на Render (429 при регистрации)

На Render очень часто не работает Gmail SMTP "из коробки" (блокировка исходящих подключений, подозрительная активность из облачных IP, проблемы с app password).

Мы сделали два улучшения:

1. При ошибке отправки письма запись в `email_verifications` теперь **удаляется**. Раньше неудачные попытки "съедали" лимит (3 попытки за 10 минут) и ты быстро получал 429, даже если проблема была в почте.
2. Для ошибок отправки почты теперь возвращается 422 (а не 429). 429 остаётся только для реального rate limit.

**Что проверить на Render:**

- В Environment Web Service все `MAIL_*` переменные должны быть заданы (особенно `MAIL_PASSWORD` — обязательно как **Secret**).
- Gmail часто не любит Render. Если после фикса всё равно не отправляется — в логах Render будет видно ошибку Mail.

Если нужно быстро протестировать регистрацию на Render без реальной почты, можно временно в `EmailVerificationService` закомментировать `Mail::raw` и просто вернуть код в ответе (для дебага). Но для продакшена лучше настроить нормальный SMTP (Brevo, Mailgun, Resend и т.п. — они лучше работают из облака).
