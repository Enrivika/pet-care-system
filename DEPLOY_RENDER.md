...
### Email (Resend.com вместо Gmail)

Gmail SMTP очень плохо работает с Render (блокировка исходящих подключений, подозрительная активность из облачных IP).

Мы полностью переключили проект на **Resend**.

**Настройка на Render (Web Service → Environment):**

- `MAIL_MAILER=resend`
- `RESEND_API_KEY=re_hpCYSSxm_7Ev7K8JpZpkXMqJ4Uu8Zy4JU` ← **обязательно как Secret** (иконка замка)
- `MAIL_FROM_ADDRESS=onboarding@resend.dev`
- `MAIL_FROM_NAME=Petopia`

После сохранения — Clear build cache + Redeploy.

**Текущий статус:**
- Используется тестовый домен `onboarding@resend.dev` (письма приходят, но часто в "Промоакции" / Spam).
- Для нормального домена позже нужно добавить и подтвердить свой домен в панели Resend (https://resend.com/domains).
- Бесплатный лимит Resend — 3000 писем/месяц (более чем достаточно).

**Сохранённые улучшения верификации:**
- При ошибке отправки запись из `email_verifications` удаляется (не копится rate-limit).
- Возвращается 422 для ошибок почты, 429 только для реального "слишком много попыток".
- Добавлен `EMAIL_VERIFICATION_DEBUG=true` (или `MAIL_MAILER=log`): коды логируются + автоматически подставляются во фронтенд. Полезно для тестов на Render.

Все остальные письма (напоминания из шедулера `reminders:send`, сброс пароля и т.д.) тоже теперь идут через Resend.

**Web Push уведомления не затронуты** — они работают через отдельный канал (`laravel-notification-channels/webpush`).

### Gmail (для справки)

Gmail оставлен в комментариях в `.env*`. Если когда-нибудь понадобится — можно вернуть, но на Render он почти всегда проблемный.
